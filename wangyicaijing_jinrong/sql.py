...
class RuKu():
    ...
    def saves(self, urls, title, date_times, froms, contents):
        print(urls)
        print(date_times)
        print(froms)
        print(title)
        print(contents)
        sql = "insert into news_types(DateTime,Url,Title,Content,Types,Forms) VALUES ('%s','%s','%s','%s','%s','%s')" % (date_times,urls,title,contents,'金融事件',froms)
        try:
